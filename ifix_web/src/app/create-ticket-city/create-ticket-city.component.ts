...
@Component({
  selector: 'app-create-ticket-city',
  templateUrl: './create-ticket-city.component.html',
  styleUrls: ['./create-ticket-city.component.css']
})
export class CreateTicketCityComponent implements OnInit {
  height: number;
  ticketsTyp: any;
  displayed = false;
  name: string;
  ...
  private ticketTypeSubscription: Subscription;
  ...
  private createbyId: string;
  private user: string;
  userName: string;
  private psDetails: any;
  groupName: string;
  private selectedCatVal = 0;
  grpLevel: number;
    ...
    this.rest.selectlocation(data).subscribe((res) => {
      ...
      if (this.respObject.success) {
        ...
        if (this.priorityLocationList.length > 0) {
          ...
          this.priorityArry = {id: this.priorityLocationList[0].recorddifftypeid, val: this.priorityId};
          this.priorityName = this.priorityLocationList[0].priority;
        }
  ...
  onCategoryChange(categorySeq, option) {
    ...
    if (opt.type !== 'header') {
      const value = opt.id;
      // console.log("SEQ",categorySeq,"Data",this.ticketTypes)
      if (this.ticketCatId) {
  ...
  afterPageLoad() {
    ...
    if (this.hascatalog !== 'Y') {
      if (this.parentPriorityId > 0) {
        ...
        // console.log(JSON.stringify(this.priorityArry));
      }
    ...
    this.rest.getrecordtypedata(ticketTypeData).subscribe((res) => {
      ...
      if (this.respObject.success) {
        ...
        if (this.tickettypeid === 0) {
          ...
          //   //   if (this.ticketsTyp[i].seqno === this.typeSeq) {
          //       // console.log("matched",this.ticketsTyp[i].seqno,"||",this.typeSeq)
          //       this.typSelected = this.ticketsTyp[0].id;
          //       this.load = true;
          //   //     break;
          //   //   }
        ...
        // console.log(JSON.stringify(this.ticketTypeArr));
        if (this.isFcategory) {
  ...
  createTicket() {
    ...
    for (let i = 0; i < dynamicFieldsmod.length; i++) {
      ...
      // if (Number(dynamicFieldsmod[i].seqno) === 38) {
      //   dynamicFieldsmod[i].value = this.locationSelected;
      // }
    ...
    // let loginGroupId
    // if(this.messageService.getLoginUserGroupId() !== null){
    //   loginGroupId = this.messageService.getLoginUserGroupId().loginGroupId;
    // } else {
    //   loginGroupId = this.userGroupId;
    // }
    ...
    //     } else {
    //       isError = true;
    //       console.log("URG",data.recordsets)
    ...
    //     } else {
    //       isError = true;
    //       console.log("IMP",data.recordsets)
        ...
        if (this.respObject.success) {
          ...
          this.rest.geturlbykey({
            ...
            if (res1.success) {
              ...
              // this.messageService.removeLoginUserGroupIdCity();
            } else {
              this.isTicketCreated = false;
