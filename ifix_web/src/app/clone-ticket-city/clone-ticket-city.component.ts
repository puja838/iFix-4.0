...
@Component({
  selector: 'app-clone-ticket-city',
  templateUrl: './clone-ticket-city.component.html',
  styleUrls: ['./clone-ticket-city.component.css']
})
export class CloneTicketCityComponent implements OnInit {
  ...
  getrecordcategorydisplay() {
    ...
    this.rest.getrecordcat(data).subscribe((res: any) => {
      if (res.success) {
        ...
        this.previouscat = this.messageService.copyArray(res.details.recordfields);
    ...
    this.rest.selectlocation(data).subscribe((res) => {
      ...
      if (this.respObject.success) {
        ...
        if (this.priorityLocationList.length > 0) {
          ...
          console.log(this.priorityId, this.priority);
        }
  ...
  cloneTicket() {
    ...
    if (!isError) {
      if (!this.messageService.blankarr(data.recordsets) && data.recordesc.trim() !== '' && data.recordname.trim() !== '' && this.rName.trim() !== '' && this.rEmail.trim() !== '' && this.rMobile.trim() !== '' && this.rLoc.trim() !== '') {
      ...
      if (!this.messageService.isBlankField(data)) {
        this.rest.createrecord(data).subscribe((res: any) => {
          ...
          if (res.success) {
            ...
            if (this.feature === 'cv') {
              this.addSingleTerm(this.RESOLUTION_COMMENT_SEQ, '', 'Ticket has been converted to : ' + res.response).then((success2) => {
                if (success2) {
                  this.addSingleTerm(this.CONVERTED_SEQ, '', 'YES').then((success) => {
                    if (success) {
