...
export class ProcessComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.displayData = {
      pageName: 'Maintain Process',
      openModalButton: 'Process',
      breadcrumb: '',
      folderName: '',
      tabName: 'Map Process'
    };
  ...
  save() {
    for(let i = 0; i<this.workingList.length;i++){
      if(this.workingList[i].id === Number(this.workingdiffid))
      {
        this.workingtypeid = this.workingList[i].recorddifftypid;
    ...
    if (!this.messageService.isBlankField(data)) {
      this.rest.createprocess(data).subscribe((res) => {
        this.respObject = res;
        if (this.respObject.success) {
