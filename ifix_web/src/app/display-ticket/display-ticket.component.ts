...
import {AngularGridInstance, Filters, Formatters, GridOption} from 'angular-slickgrid';
...
export class DisplayTicketComponent implements OnInit, OnDestroy {
  height: number;
  solHeight: number;
  dataLoaded: boolean;
  ...
  dataLoadedForFaCheck: boolean;
  dataLoadedForAssets: boolean;
  dataLoadedForAssignedUser: boolean;
  dataLoadedForAddWorkNote: boolean;
  dataLoadedForAddButton: boolean;
  dataLoadedForAddEffort: boolean;
  dataLoadedForAddAttachToParentSearch: boolean;
  dataLoadedForAddAttachToParentAttach: boolean;
  dataLoadedForAttachToParentGrid: boolean;
  dataLoadedForSendMail: boolean;
  dataLoadedForAddApplyButton: boolean;
  ...
  constructor(private messageService: MessageService, private rest: RestApiService, private route: Router, private notifier: NotifierService,
              ...
              private rating: NgbRatingConfig, private socketService: SocketService) {

    this.modifiedAssetSubscribe = this.messageService.getModifiedAssetData().subscribe((modifiedAssetDetails) => {
  ...
  ngOnInit() {
    ...
    this.dataLoadedForFaCheck = false;
    this.dataLoadedForAssets = false;
    this.dataLoadedForAssignedUser = false;
    this.dataLoadedForAddWorkNote = false;
    this.dataLoadedForAddButton = false;
    this.dataLoadedForAddEffort = false;
    this.dataLoadedForAddAttachToParentSearch = false;
    this.dataLoadedForAddAttachToParentAttach = false;
    this.dataLoadedForAttachToParentGrid = false;
    this.dataLoadedForSendMail = false;
    this.dataLoadedForAddApplyButton = false;
  ...
  getTicketDetails() {
    ...
    this.rest.getrecorddetails({
      ...
      if (res1.success) {
        ...
        // this.messageService.firstname = this.rName;
        // this.messageService.lastname = '';
        // this.messageService.email = this.rEmail;
        // this.messageService.mobile = this.rMobile;
        // this.messageService.branch = this.rLoc;
          ...
          if (res.success) {
            if (res.details.length > 0) {
              ...
              if (this.statusseq === this.RESOLVE_STATUS_SEQUENCE || this.statusseq === this.CLOSE_STATUS_SEQ
                ...
                || this.statusseq === this.REJECTED_STATUS_SEQ || this.statusseq === this.CANCEL_STATUS_SEQ) {
  ...
  startDisplay() {
    ...
    this.socketService.emit('ticketRoomJoin', {ticketid: this.tId, groupid: this.userGroupId});
    ...
    this.rest.insertuserticket(data).subscribe((res: any) => {
      ...
      this.getTab().then((val: any) => {
        ...
        // console.log('canChangeUser:', this.canChangeUser, this.isDisabled);
        ...
        console.log(this.updateStatusBtn);
    ...
    this.rest.getrecordcat(data).subscribe((res: any) => {
      ...
      if (res.success) {
        ...
        if (this.priorityType === 1) {
          this.impacts = res.details.recordimpact;
          this.urgencies = res.details.recordurgency;
          this.impactid = this.ticketDetails[0].impactid;
          this.urgencyid = this.ticketDetails[0].urgencyid;
        }
        if (res.details.estimatedefforts) {
          this.estimatedEffortName = res.details.estimatedefforts[0];
        }
        if (res.details.changetypes) {
          this.crtype = res.details.changetypes[0];
        }
        // console.log(this.priorityType, this.grpLevel, this.displayPriority);
      } else {
        this.notifier.notify('error', res.message);
      }
    }, (err) => {
    ...
    this.rest.gettransitiongroupdetails({
      ...
      if (res.success) {
        ...
        if (res.details.length > 0) {
          ...
          // console.log(JSON.stringify(res.details));
          this.stategroups = this.messageService.sortByKey(res.details, 'groupname');
          // console.log(JSON.stringify(this.stategroups),this.grpLevel)
          ...
          // console.log(this.grpSelected, this.agroupid);
    ...
    if (event.tab.textLabel === 'Ticket Details') {
      ...
      this.rest.getparencollaborationtchildlogs(data).subscribe((res: any) => {
        if (res.success) {
          ...
          this.parentChildCollabArr.sort((a, b) => {
            const keyA = new Date(a.createddate);
            const keyB = new Date(b.createddate);
            // Compare the 2 dates
            if (keyA > keyB) {
              return -1;
            }
      ...
      if (this.createUrl === '') {
        ...
        this.createUrl = this.messageService.getSafeUrl(this.createUrl);
        this.rest.geturlbykey({
          ...
          if (res.success) {
            if (res.details.length > 0) {
              ...
              // this.messageService.setLoginUserGroupId({'loginGroupId': Number(this.userGroupId)});
  ...
  onImpactChange(index) {
    if (this.urgencyid !== 0) {
      const data = {
        ...
        'mstorgnhirarchyid': this.orgId,
        'recordtypeid': this.typeChecked,
        'recordimpactid': Number(this.impactid),
        'recordurgencyid': Number(this.urgencyid)
      };
      this.getPriority(data).then((success) => {
        if (success) {
          this.changePriority().then((success1) => {
            if (success1) {
              if (this.hasSLA && this.grpLevel > 1) {
                this.terminateWorker();
  ...
  changeUserWithState() {
    ...
    if (this.userSelected > 0) {
      this.dataLoadedForAssignedUser = true;
    ...
    const promise = new Promise((resolve, reject) => {
      if (Number(this.grpSelected) === this.agroupid && Number(this.userSelected) === this.auserid) {
        ...
        const data = {
          ...
          samegroup: sameGroup,
          creatorgroupid: this.creatorgrpid
        };
        // console.log(JSON.stringify(data));
        this.rest.changerecordgroup(data).subscribe((res: any) => {
          ...
          if (res.success) {
            ...
            console.log(this.isSameGroup, this.userGroupId, this.agroupid, this.canChangeUser);
            this.messageService.setAssignedData({auserid: this.auserid, agroupid: this.agroupid});
            this.alltermsValue();
            resolve(true);
          } else {
  ...
  addTermValue(type) {
    this.dataLoadedForAddWorkNote = true;
    ...
    if (this.comment !== '') {
      ...
      this.dataLoadedForAddWorkNote = false;
      this.notifier.notify('error', 'Please add any work note.');
    }
  }
  ...
  ngOnDestroy(): void {
    ...
    this.socketService.emit('ticketRoomLeave', this.tId);
  }
  ...
  clearUserTicket(ticketid) {
    const data = {
      recordid: ticketid,
      groupid: this.userGroupId,
      userid: this.messageService.getUserId()
    };
  ...
  searchTicket() {
    if (this.tNumber.trim() !== this.ticketId) {
      ...
      this.dataLoadedForAttachToParentGrid = true;
  ...
  searchParentTicket() {
    if (this.tNumber.trim() !== this.ticketId) {
      ...
      this.dataLoadedForAddAttachToParentSearch = true;
  ...
  updateTicket() {
    this.dataLoadedForFaCheck = true;
  ...
  applyFilter() {
    this.previousTerms = [];
    this.dataLoadedForAddApplyButton = true;
  ...
  cloneTicket(type) {
    if (this.grpLevel === 1) {
      ...
      // this.messageService.changeRouting(this.messageService.cloneUrl, {
      //   id: this.tId,
      //   code: this.ticketId
      // });
    } else {
    ...
    this.rest.geturlbykey({
      ...
      if (res.success) {
        if (res.details.length > 0) {
          ...
          this.messageService.setNavigation(this.url);
      ...
      this.addSingleTerm(type, '', this.commentTerm).then((success) => {
        ...
        if (success) {
          ...
          if (this.termopentype === 'priority') {
            ...
            this.addSingleTerm(this.EFFORT_SEQ, '', this.selectedTime).then((success1) => {
              if (success1) {
  ...
  openvendorticketterm() {
    ...
    // this.closeCommentTermDialog();
    // this.addvendortickettermdialog.afterClosed().subscribe(result => {
    //   console.log('close modal' + result);
    //   if (result !== 'dynamic') {
      ...
      if (!this.messageService.isBlankField(data)) {
        ...
        this.rest.childsearchcriteria(data).subscribe((res: any) => {
          ...
          if (res.success) {
            ...
            if (res.details.length > 0) {
              for (let i = 0; i < res.details.length; i++) {
                if (res.details[0].statusseqno === this.CANCEL_STATUS_SEQ ||
                  res.details[0].statusseqno === this.CLOSE_STATUS_SEQ ||
                  res.details[0].statusseqno === this.RESOLVE_STATUS_SEQUENCE) {
                  matched = true;
                }
              }
              if (matched) {
  ...
  attachParentTicket() {
    this.isAttachedTicket = true;
    this.dataLoadedForAddAttachToParentAttach = true;
    const data = {
      'clientid': this.clientId,
      'mstorgnhirarchyid': this.orgId,
      'childid': this.tId,
