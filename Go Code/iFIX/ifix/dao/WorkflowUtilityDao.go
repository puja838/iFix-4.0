...
func(mdao DbConn) Updatestagingdetailswithouttran(tz *entities.StagingUtilityEntity) error {
	logger.Log.Print("Updatestagingdetailswithouttran ", tz)
	log.Print("Updatestagingdetailswithouttran", tz)
	...
	defer stmt.Close()
	_, err = stmt.Exec(tz.Assignedloginname, tz.Assignedgroupid, tz.Assignedgroup, tz.Assigneduserid, tz.Assigneduser, tz.Lastuserid, tz.Lastuser, tz.Reassigncount,tz.Clientid,tz.Mstorgnhirarchyid, tz.Recordid)
	if err != nil {
		logger.Log.Print("Updatestagingdetails Execute Statement  Error", err)
		log.Print("Updatestagingdetails Execute Statement  Error", err)
		return err
