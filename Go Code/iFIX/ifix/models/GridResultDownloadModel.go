...
func GridResultToExcelConverter(tz *entities.ResultGridRequestEntity) (string, string, bool, error, string) {
	logger.Log.Println("Inside GridResultToExcelConverter Model")
	lock.Lock()
	defer lock.Unlock()
	...
	resp, err := http.Post(config.GridResultUrl, "application/json", bytes.NewBuffer(sendData))
	if err != nil {
		logger.Log.Println(err)
		return "", "", false, err, "Unable to Request For Data"
	}
	var result entities.JsonToExcelGridResponse
	...
	if len(result.Details.RequestResultGridData) == 0 {
		logger.Log.Println("No Data Is There")
		return "", "", false, err, "No Records Found"
	}
	//logger.Log.Println(result)

	_, filePath, err := utility.GridResultToExcelConverter(result, tz)
	if err != nil {
		return "", "", false, err, "Something Went Wrong"
	}
