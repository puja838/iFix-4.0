...
func MoveWorkflowwithapi(tz *entities.Workflowentity) (int64, bool, error, string) {
	...
	if tz.Previousstateid > -1 {
		...
		logger.Log.Println("checkCurrentState========================================", checkCurrentState[0].Currentstateid, tz.Previousstateid)
...
func Getlastactionerdetails(ticketid int64, groupid int64, db *sql.DB) (string, string, string, int64, int64, bool, error) {
	...
	details, err := dataAccess.Getlastactionerwithgroup(requestid, groupid)
	if err != nil {
		return "", "", "", 0, 0, false, err
	}
	lastgroupname := ""
	...
	if len(details) > 0 {
		lastgroupname = details[0].Lastgroupname
		lastusername = details[0].Lastusername
		lastgroupid = details[0].Lastgroupid
		lastuserloginname = details[0].Lastuserloginname
		lastuserid = details[0].Userid
	} else {
...
func Detachchildticket(tz *entities.Workflowentity) (int64, bool, error, string) {
	...
	if len(childhistory) > 0 {
		...
		*/
		...
		//	return 0, false, err, ""
		//}
		//currentstate, err := dataAccess.Getstatebystatusseq(childhistory[0].Clientid, childhistory[0].Mstorgnhirarchyid, config.NEW_SEQ)
		//if err != nil {
		//	return 0, false, err, "Something went wrong"
		//}
		//if len(currentstate) == 0 {
		//	return 0, false, err, "SLA can't be start from beginning."
		//}
