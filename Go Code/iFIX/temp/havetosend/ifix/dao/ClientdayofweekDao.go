...
var insertClientdayofweek = "INSERT INTO mstclientdayofweek (clientid, mstorgnhirarchyid, dayofweekid, starttimeinteger, starttime, endtimeinteger, endtime, nextdayforward) VALUES (?,?,?,?,?,?,?,?)"
...
func (dbc DbConn) InsertClientdayofweek(tz *entities.ClientdayofweekEntity, tz1 *entities.ClientdayofweekdetailsEntity) (int64, error) {
	logger.Log.Println("In side InsertClientdayofweek")
	logger.Log.Println("Query -->", insertClientdayofweek)
	stmt, err := dbc.DB.Prepare(insertClientdayofweek)
	defer stmt.Close()
	if err != nil {
		logger.Log.Println("InsertClientdayofweek Prepare Statement  Error", err)
		return 0, err
	}
	logger.Log.Println("Parameter -->", tz.Clientid, tz.Mstorgnhirarchyid, tz1.Dayofweekid, tz1.Starttimeinteger, tz1.Starttime, tz1.Endtimeinteger, tz1.Endtime, tz1.Nextdayforward)
	res, err := stmt.Exec(tz.Clientid, tz.Mstorgnhirarchyid, tz1.Dayofweekid, tz1.Starttimeinteger, tz1.Starttime, tz1.Endtimeinteger, tz1.Endtime, tz1.Nextdayforward)
	if err != nil {
...
func (dbc DbConn) GetAllClientdayofweek(page *entities.ClientdayofweekEntity, OrgnType int64) ([]entities.ClientdayofweekresponseEntity, error) {
	...
	if OrgnType == 1 {
		getClientdayofweek = "SELECT a.id as ID,a.clientid as Clientid, a.mstorgnhirarchyid as Mstorgnhirarchyid,a.dayofweekid as Dayofweekid, a.starttimeinteger as Starttimeinteger, a.starttime as Starttime, a.endtimeinteger as Endtimeinteger, a.endtime as Endtime, a.nextdayforward as Nextdayforward, a.activeflg as Activeflg,b.name as Clientname,c.name as Mstorgnhirarchyname FROM mstclientdayofweek a,mstclient b,mstorgnhierarchy c WHERE a.deleteflg =0 and a.activeflg=1 and a.clientid =b.id AND a.mstorgnhirarchyid = c.id ORDER BY a.id DESC LIMIT ?,?"
		params = append(params, page.Offset)
		params = append(params, page.Limit)
	} else if OrgnType == 2 {
		getClientdayofweek = "SELECT a.id as ID,a.clientid as Clientid, a.mstorgnhirarchyid as Mstorgnhirarchyid,a.dayofweekid as Dayofweekid, a.starttimeinteger as Starttimeinteger, a.starttime as Starttime, a.endtimeinteger as Endtimeinteger, a.endtime as Endtime, a.nextdayforward as Nextdayforward, a.activeflg as Activeflg,b.name as Clientname,c.name as Mstorgnhirarchyname FROM mstclientdayofweek a,mstclient b,mstorgnhierarchy c WHERE a.clientid = ? AND a.deleteflg =0 and a.activeflg=1 and a.clientid =b.id AND a.mstorgnhirarchyid = c.id ORDER BY a.id DESC LIMIT ?,?"
	...
	for rows.Next() {
		...
		rows.Scan(&value.ID, &value.Clientid, &value.Mstorgnhirarchyid, &value.Dayofweekid, &value.Starttimeinteger, &value.Starttime, &value.Endtimeinteger, &value.Endtime, &value.Nextdayforward, &value.Activeflg, &value.Clientname, &value.Mstorgnhirarchyname)
