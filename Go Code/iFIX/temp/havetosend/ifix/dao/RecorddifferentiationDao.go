...
var getdifferentiationname = "select id as Id,name as Recorddiffname,COALESCE(parentcategorynames,'') AS Parentcatname from mstrecorddifferentiation where clientid =? and mstorgnhirarchyid=? and name like ? and deleteflg=0 and activeflg=1 and recorddifftypeid=?"
