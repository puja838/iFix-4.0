...
func (dbc DbConn) Getrowbygrpid(page *entities.ClientsupportgroupnewEntity) (*entities.ClientsupportgroupnewEntity, error) {
	...
	for rows.Next() {
		//value := entities.ClientsupportgroupnewEntity{}
		rows.Scan(&value.Clientid, &value.Mstorgnhirarchyid, &value.Supportgroupid, &value.Supportgrouplevelid, &value.Mstclienttimezoneid, &value.Reporttimezoneid, &value.Email, &value.Isworkflow, &value.Hascatalog)
		//values = append(values, value)
		break
