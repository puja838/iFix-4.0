...
var duplicateUidGen = "SELECT count(id) total FROM  uidgen WHERE clientid = ? AND mstorgnhirarchyid = ? AND difftypeid = ? AND code = ? AND uid = ? AND deleteflg = 0"
...
func (dbc DbConn) CheckDuplicateUidGen(tz *entities.UidGenEntity) (entities.UidGenEntities, error) {
	...
	err := dbc.DB.QueryRow(duplicateUidGen, tz.Clientid, tz.Mstorgnhirarchyid, tz.Difftypeid, tz.Code, tz.Uid).Scan(&value.Total)
	switch err {
	case sql.ErrNoRows:
		value.Total = 0
		return value, nil
...
func (dbc DbConn) InsertUidGen(tz *entities.UidGenEntity) (int64, error) {
	...
	logger.Log.Println("Parameter -->", tz.Clientid, tz.Mstorgnhirarchyid, tz.Difftypeid, tz.Code, tz.Uid)
	res, err := stmt.Exec(tz.Clientid, tz.Mstorgnhirarchyid, tz.Difftypeid, tz.Code, tz.Uid)
	if err != nil {
...
func (dbc DbConn) GetAllUidGen(tz *entities.UidGenEntity, OrgnType int64) ([]entities.UidGenEntity, error) {
	...
	for rows.Next() {
		...
		rows.Scan(&value.Id, &value.Clientid, &value.Mstorgnhirarchyid, &value.Difftypeid, &value.Code, &value.Uid, &value.Activeflg, &value.Clientname, &value.Mstorgnhirarchyname, &value.Difftypename)
...
func (dbc DbConn) UpdateUidGen(tz *entities.UidGenEntity) error {
	...
	_, err = stmt.Exec(tz.Mstorgnhirarchyid, tz.Difftypeid, tz.Code, tz.Uid, tz.Id)
	if err != nil {
...
func (dbc DbConn) UpdateUidGenforemail(tz *entities.UidGenEntity) error {
	...
	_, err = stmt.Exec(tz.Uid, tz.Difftypeid)
	if err != nil {
		logger.Log.Println("UpdateUidGen Execute Statement  Error", err)
		return err
	}
