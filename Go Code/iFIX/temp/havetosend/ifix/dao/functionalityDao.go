...
var insertmapping = "INSERT into mapfunctionality(clientid,mstorgnhirarchyid,funcid,funcdescid,description,seqno,colorcode,image,ismanegerialview) values(?,?,?,?,?,?,?,?,?)"
...
var duplicatemapping = "SELECT count(id) total from mapfunctionality where clientid=? and mstorgnhirarchyid=? and funcid=?  and description=? AND funcdescid=?  and deleteflg=0"
var mappingbytype = "SELECT funcdescid,description,seqno,colorcode,image,ismanegerialview from mapfunctionality where clientid=? and mstorgnhirarchyid=? and funcid=? and activeflg=1 and deleteflg=0 order by description"
var mappingdetails = "SELECT a.id ,a.description,a.activeflg,a.image,a.ismanegerialview,b.name as Clientname, d.name as Orgname,c.name as funcname from mapfunctionality a,mstclient b, mstorgnhierarchy d,mstfunctionality c where a.clientid = b.id AND a.mstorgnhirarchyid = d.id AND a.funcid=c.id and a.activeflg=1 and a.deleteflg=0 ORDER BY a.id DESC LIMIT ?,? "
...
func (mdao DbConn) Checkduplicatefuncmapping(tz *entities.FuncmappingEntity) (entities.FuncmappingEntitities, error) {
	...
	err := mdao.DB.QueryRow(duplicatemapping, tz.Clientid, tz.Mstorgnhirarchyid, tz.Funcid, tz.Description, tz.Funcdescid).Scan(&value.Total)
	switch err {
	case sql.ErrNoRows:
		value.Total = 0
		return value, nil
...
func (mdao DbConn) Insertfuncmapping(tz *entities.FuncmappingEntity) (int64, error) {
	...
	res, err := stmt.Exec(tz.Clientid, tz.Mstorgnhirarchyid, tz.Funcid, tz.Funcdescid, tz.Description, tz.Seqno, tz.Colorcode, tz.Image, tz.Ismanegerialview)
	if err != nil {
...
func (mdao DbConn) Getfuncmappingdetails(tz *entities.FuncmappingEntity) ([]entities.FuncmappingRespEntity, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&value.ID, &value.Description, &value.Activeflg, &value.Image, &value.Ismanegerialview, &value.Clientname, &value.Orgname, &value.Funcname)
		if err != nil {
