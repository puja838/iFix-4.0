...
var insertMstslafullfillmentcriteria = "INSERT INTO mstslafullfillmentcriteria (clientid, mstorgnhirarchyid, slaid, mstrecorddifferentiationtickettypeid, mstrecorddifferentiationpriorityid, mstrecorddifferentiationworkingcatid, responsetimeinhr, responsetimeinmin, responsetimeinsec, resolutiontimeinhr, resolutiontimeinmin, resolutiontimeinsec, supportgroupspecific) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"
var duplicateMstslafullfillmentcriteria = "SELECT count(id) total FROM  mstslafullfillmentcriteria WHERE clientid = ? AND mstorgnhirarchyid = ? AND slaid = ? AND mstrecorddifferentiationtickettypeid = ? AND mstrecorddifferentiationpriorityid = ? AND mstrecorddifferentiationworkingcatid = ? AND responsetimeinhr = ? AND responsetimeinmin = ? AND responsetimeinsec = ? AND resolutiontimeinhr = ? AND resolutiontimeinmin = ? AND resolutiontimeinsec = ? AND supportgroupspecific = ? AND deleteflg = 0"
...
var updateMstslafullfillmentcriteria = "UPDATE mstslafullfillmentcriteria SET mstorgnhirarchyid = ?, slaid = ?, mstrecorddifferentiationtickettypeid = ?, mstrecorddifferentiationpriorityid = ?, mstrecorddifferentiationworkingcatid = ?, responsetimeinhr = ?, responsetimeinmin = ?, responsetimeinsec = ?, resolutiontimeinhr = ?, resolutiontimeinmin = ?, resolutiontimeinsec = ?, supportgroupspecific = ? WHERE id = ? "
var deleteMstslafullfillmentcriteria = "UPDATE mstslafullfillmentcriteria SET deleteflg = '1' WHERE id = ? "
var duplicateMstslafullfillmentcriteriawithoutcat = "SELECT count(id) total FROM  mstslafullfillmentcriteria WHERE clientid = ? AND mstorgnhirarchyid = ? AND slaid = ? AND mstrecorddifferentiationtickettypeid = ? AND mstrecorddifferentiationpriorityid = ? AND responsetimeinhr = ? AND responsetimeinmin = ? AND responsetimeinsec = ? AND resolutiontimeinhr = ? AND resolutiontimeinmin = ? AND resolutiontimeinsec = ? AND supportgroupspecific = ? AND deleteflg = 0"
...
func (dbc DbConn) CheckDuplicateMstslafullfillmentcriteria(tz *entities.MstslafullfillmentcriteriaEntity) (entities.MstslafullfillmentcriteriaEntities, error) {
	...
	err := dbc.DB.QueryRow(duplicateMstslafullfillmentcriteria, tz.Clientid, tz.Mstorgnhirarchyid, tz.Slaid, tz.Mstrecorddifferentiationtickettypeid, tz.Mstrecorddifferentiationpriorityid, tz.Mstrecorddifferentiationworkingcatid, tz.Responsetimeinhr, tz.Responsetimeinmin, tz.Responsetimeinsec, tz.Resolutiontimeinhr, tz.Resolutiontimeinmin, tz.Resolutiontimeinsec, tz.Supportgroupspecific).Scan(&value.Total)
	switch err {
	case sql.ErrNoRows:
		value.Total = 0
		return value, nil
...
func (dbc DbConn) CheckDuplicateMstslafullfillmentcriteriawithoutcat(tz *entities.MstslafullfillmentcriteriaEntity) (entities.MstslafullfillmentcriteriaEntities, error) {
	logger.Log.Println("In side CheckDuplicateMstslafullfillmentcriteria")
	value := entities.MstslafullfillmentcriteriaEntities{}
	err := dbc.DB.QueryRow(duplicateMstslafullfillmentcriteriawithoutcat, tz.Clientid, tz.Mstorgnhirarchyid, tz.Slaid, tz.Mstrecorddifferentiationtickettypeid, tz.Mstrecorddifferentiationpriorityid, tz.Responsetimeinhr, tz.Responsetimeinmin, tz.Responsetimeinsec, tz.Resolutiontimeinhr, tz.Resolutiontimeinmin, tz.Resolutiontimeinsec, tz.Supportgroupspecific).Scan(&value.Total)
	switch err {
	case sql.ErrNoRows:
		value.Total = 0
		return value, nil
...
func (dbc DbConn) InsertMstslafullfillmentcriteria(tz *entities.MstslafullfillmentcriteriaEntity) (int64, error) {
	...
	logger.Log.Println("Parameter -->", tz.Clientid, tz.Mstorgnhirarchyid, tz.Slaid, tz.Mstrecorddifferentiationtickettypeid, tz.Mstrecorddifferentiationpriorityid, tz.Mstrecorddifferentiationworkingcatid, tz.Responsetimeinhr, tz.Responsetimeinmin, tz.Responsetimeinsec, tz.Resolutiontimeinhr, tz.Resolutiontimeinmin, tz.Resolutiontimeinsec, tz.Supportgroupspecific)
...
func (dbc DbConn) GetAllMstslafullfillmentcriteria(page *entities.MstslafullfillmentcriteriaEntity) ([]entities.MstslafullfillmentcriteriaEntity, error) {
...
func (dbc DbConn) UpdateMstslafullfillmentcriteria(tz *entities.MstslafullfillmentcriteriaEntity) error {
	...
	_, err = stmt.Exec(tz.Mstorgnhirarchyid, tz.Slaid, tz.Mstrecorddifferentiationtickettypeid, tz.Mstrecorddifferentiationpriorityid, tz.Mstrecorddifferentiationworkingcatid, tz.Responsetimeinhr, tz.Responsetimeinmin, tz.Responsetimeinsec, tz.Resolutiontimeinhr, tz.Resolutiontimeinmin, tz.Resolutiontimeinsec, tz.Supportgroupspecific, tz.Id)
	if err != nil {
...
func (dbc DbConn) GetMstslafullfillmentcriteriaCount(tz *entities.MstslafullfillmentcriteriaEntity) (entities.MstslafullfillmentcriteriaEntities, error) {
