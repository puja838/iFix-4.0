...
var getdifferentiationname = "select id as Id,name as Recorddiffname,COALESCE(parentcategorynames,'') AS Parentcatname from mstrecorddifferentiation where clientid =? and mstorgnhirarchyid=? and name like ? and deleteflg=0 and activeflg=1"
...
func (dbc DbConn) GetRecorddifferentiationname(page *entities.RecorddifferentiationEntity) ([]entities.RecorddifferentiationnameEntity, error) {
	...
	rows, err := dbc.DB.Query(getdifferentiationname, page.Clientid, page.Mstorgnhirarchyid, "%"+page.Name+"%")
	defer rows.Close()
	if err != nil {
		logger.Log.Println("GetAllRecorddifferentiation Get Statement Prepare Error", err)
		return values, err
...
func (dbc DbConn) GetRecorddifferentiationoldname(ID int64) (string, error) {
	...
	for rows.Next() {
		err = rows.Scan(&Diffname)
		logger.Log.Println("GetRecorddifferentiationoldname rows.next() Error", err)
	}
	return Diffname, nil
}

func (dbc DbConn) Updateparentcategorynames(ClientID int64, OrgnID int64, ID int64, Oldname string, Newname string) error {
	logger.Log.Println("Updatestageid parameters -->", ClientID, OrgnID, ID)
	var sql = "update mstrecorddifferentiation SET parentcategorynames = replace(parentcategorynames,?,?) where clientid=?  AND mstorgnhirarchyid=? AND (parentcategoryids like ? or parentcategoryids like ? or parentcategoryids like ?) and id >0"
	...
	//'%->946->%'
	defer stmt.Close()
	_, err = stmt.Exec(Oldname, Newname, ClientID, OrgnID, "%->"+strconv.FormatInt(ID, 10), "%->"+strconv.FormatInt(ID, 10)+"->%", strconv.FormatInt(ID, 10)+"->%")
	if err != nil {
		logger.Log.Println(err)
