...
func (dbc DbConn) CheckDuplicateNotificationTemplate(tz *entities.MstNotificationTemplateEntity) (int64, error) {
	...
	if tz.ChannelTypeID == 1 {
		var query = "SELECT COUNT(id) Total FROM mstnotificationtemplate WHERE clientid=? AND mstorgnhirarchyid=? AND recordtypeid=? AND workingcategoryid=? AND channeltype=? AND subjectortitle=? AND eventtype=? AND eventparams=? AND activeflg=1 AND deleteflg=0"
		...
		err = dbc.DB.QueryRow(query, tz.ClientID, tz.MstOrgnHirarchyID, tz.RecordTypeID, tz.WorkingCategoryID, tz.ChannelTypeID, tz.SubjectOrTitle, tz.EventTypeID, string(b)).Scan(&value)
		switch err {
		case sql.ErrNoRows:
			value = 0
			return value, nil
...
func (dbc DbConn) InsertNotificationTemplate(tz *entities.MstNotificationTemplateEntity) (int64, error) {
	...
	if tz.ChannelTypeID == 1 {
		query = "INSERT INTO mstnotificationtemplate(clientid, mstorgnhirarchyid, recordtypeid, workingcategoryid, channeltype, subjectortitle, body, additionalrecipient,sendtocreator,sendtooriginalcreator,sendtoassignee,sendtoassigneegroup,sendtoassigneegroupmember, eventtype, eventparams,activeflg, deleteflg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,0)"
		logger.Log.Println("Query -->", query)
		...
		logger.Log.Println("Parameter -->", tz.ClientID, tz.MstOrgnHirarchyID, tz.RecordTypeID, tz.WorkingCategoryID, tz.ChannelTypeID, tz.SubjectOrTitle, tz.Body, tz.AdditionalRecipient, tz.SendToCreator, tz.SendToOrgCreator, tz.SendToAssignee, tz.SendToAssigneeGroup, tz.SendToAssigneeGroupMember, tz.EventTypeID, string(b))
		res, err := stmt.Exec(tz.ClientID, tz.MstOrgnHirarchyID, tz.RecordTypeID, tz.WorkingCategoryID, tz.ChannelTypeID, tz.SubjectOrTitle, tz.Body, tz.AdditionalRecipient, tz.SendToCreator, tz.SendToOrgCreator, tz.SendToAssignee, tz.SendToAssigneeGroup, tz.SendToAssigneeGroupMember, tz.EventTypeID, string(b))
		if err != nil {
		...
		query = "INSERT INTO mstnotificationtemplate(clientid, mstorgnhirarchyid, recordtypeid, workingcategoryid, channeltype,body,additionalrecipient,sendtocreator,sendtooriginalcreator,sendtoassignee,sendtoassigneegroup,sendtoassigneegroupmember, eventtype, eventparams,smstemplateid,smstype,activeflg, deleteflg) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,0)"
		logger.Log.Println("Query -->", query)
		stmt, err := dbc.DB.Prepare(query)
		...
		logger.Log.Println("Parameter -->", tz.ClientID, tz.MstOrgnHirarchyID, tz.RecordTypeID, tz.WorkingCategoryID, tz.ChannelTypeID, tz.Body, tz.AdditionalRecipient, tz.SendToCreator, tz.SendToOrgCreator, tz.SendToAssignee, tz.SendToAssigneeGroup, tz.SendToAssigneeGroupMember, tz.EventTypeID, string(b), tz.SmsTemplateID, tz.SmsTypeID)
		res, err := stmt.Exec(tz.ClientID, tz.MstOrgnHirarchyID, tz.RecordTypeID, tz.WorkingCategoryID, tz.ChannelTypeID, tz.Body, tz.AdditionalRecipient, tz.SendToCreator, tz.SendToOrgCreator, tz.SendToAssignee, tz.SendToAssigneeGroup, tz.SendToAssigneeGroupMember, tz.EventTypeID, string(b), tz.SmsTemplateID, tz.SmsTypeID)
		if err != nil {
			logger.Log.Println("InsertNotificationTemplate Execute Statement  Error", err)
			return 0, err
		}
...
//grid API
func (dbc DbConn) GetAllNotificationTemplates(page *entities.MstNotificationTemplateEntity) ([]entities.MstNotificationTemplateEntity, error) {
	...
	var query = "SELECT mstnotificationtemplate.id,mstnotificationtemplate.clientid,mstclient.name clientname, " +
		...
		" IF(mstnotificationtemplate.smstype=1,'LongSMS','N/A') smstype,mstnotificationtemplate.activeflg FROM mstnotificationtemplate JOIN mstclient ON mstnotificationtemplate.clientid=mstclient.id JOIN mstorgnhierarchy ON" +
	...
	for rows.Next() {
		...
		var eparams string
		err = rows.Scan(&value.ID, &value.ClientID, &value.ClientName, &value.MstOrgnHirarchyID, &value.MstOrgnHirarchyName, &value.RecordTypeTypeID, &value.RecordTypeType, &value.RecordTypeID, &value.RecordType, &value.WorkingCategoryTypeID, &value.WorkingCategoryType, &value.WorkingCategoryID, &value.WorkingCategory, &value.ChannelTypeID, &value.ChannelType, &value.SubjectOrTitle, &value.Body, &value.AdditionalRecipient, &value.SendToCreator, &value.SendToOrgCreator, &value.SendToAssignee, &value.SendToAssigneeGroup, &value.SendToAssigneeGroupMember, &value.EventTypeID, &value.EventType, &eparams, &value.SmsTemplateID, &value.SmsTypeID, &value.SmsType, &value.Activeflg)
		if err != nil {
			log.Println("err", err)
...
func (dbc DbConn) GetAllMsttemplate(page *entities.MsttemplateEntity) ([]entities.MsttemplateEntity, error) {
...
func (dbc DbConn) UpdateNotificationTemplate(tz *entities.MstNotificationTemplateEntity) error {
	...
	if tz.ChannelTypeID == 1 {
		var query = "UPDATE mstnotificationtemplate SET channeltype=?,subjectortitle=?,body=?,additionalrecipient=?,sendtocreator=?,sendtooriginalcreator=?,sendtoassignee=?,sendtoassigneegroup=?,sendtoassigneegroupmember=?,eventtype=?,eventparams=? WHERE id=? AND clientid=? AND mstorgnhirarchyid=?"
		stmt, err := dbc.DB.Prepare(query)
		...
		_, err = stmt.Exec(tz.ChannelTypeID, tz.SubjectOrTitle, tz.Body, tz.AdditionalRecipient, tz.SendToCreator, tz.SendToOrgCreator, tz.SendToAssignee, tz.SendToAssigneeGroup, tz.SendToAssigneeGroupMember, tz.EventTypeID, string(b), tz.ID, tz.ClientID, tz.MstOrgnHirarchyID)
		if err != nil {
			logger.Log.Println("UpdateMsttemplate Execute Statement  Error", err)
			return err
		}
		...
		var query = "UPDATE mstnotificationtemplate SET channeltype=?,additionalrecipient=?,body=?,sendtocreator=?,sendtooriginalcreator=?,sendtoassignee=?,sendtoassigneegroup=?,sendtoassigneegroupmember=?,eventtype=?,eventparams=?,smstemplateid=?,smstype=? WHERE id=? AND clientid=? AND mstorgnhirarchyid=?"
		stmt, err := dbc.DB.Prepare(query)
		...
		_, err = stmt.Exec(tz.ChannelTypeID, tz.AdditionalRecipient, tz.Body, tz.SendToCreator, tz.SendToOrgCreator, tz.SendToAssignee, tz.SendToAssigneeGroup, tz.SendToAssigneeGroupMember, tz.EventTypeID, string(b), tz.SmsTemplateID, tz.SmsTypeID, tz.ID, tz.ClientID, tz.MstOrgnHirarchyID)
		if err != nil {
			logger.Log.Println("UpdateMsttemplate Execute Statement  Error", err)
			return err
		}
...
func (dbc DbConn) GetNotificationTemplateCount(tz *entities.MstNotificationTemplateEntity) (int64, error) {
...
func (dbc DbConn) GetMsttemplateCount(tz *entities.MsttemplateEntity) (entities.MsttemplateEntities, error) {
