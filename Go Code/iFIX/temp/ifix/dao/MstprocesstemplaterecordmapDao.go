...
var insertMstprocesstemplaterecordmap = "INSERT INTO mstprocesstemplaterecordmap (clientid, mstorgnhirarchyid, recorddifftypeid, recorddiffid, mstprocessid) VALUES (?,?,?,?,?)"
var duplicateMstprocesstemplaterecordmap = "SELECT count(id) total FROM  mstprocesstemplaterecordmap WHERE clientid = ? AND mstorgnhirarchyid = ? AND recorddifftypeid = ? AND recorddiffid = ? and mstprocessid=?  AND deleteflg = 0 and activeflg=1"
...
var updateMstprocesstemplaterecordmap = "UPDATE mstprocesstemplaterecordmap SET mstorgnhirarchyid = ?, recorddifftypeid = ?, recorddiffid = ? WHERE id = ? "
var deleteMstprocesstemplaterecordmap = "UPDATE mstprocesstemplaterecordmap SET deleteflg = '1' WHERE id = ? "

func (dbc DbConn) CheckDuplicateMstprocesstemplaterecordmap(tz *entities.MstprocessEntity,processid int64) (entities.MstprocessrecordmapEntities, error) {
	logger.Log.Println("In side CheckDuplicateMstprocessrecordmap")
	value := entities.MstprocessrecordmapEntities{}
	err := dbc.DB.QueryRow(duplicateMstprocessrecordmap, tz.Clientid, tz.Mstorgnhirarchyid, tz.Recorddifftypeid, tz.Recorddiffid,processid).Scan(&value.Total)
	switch err {
	case sql.ErrNoRows:
		value.Total = 0
		return value, nil
...
*/
func InsertMstprocesstemplaterecordmapwithtransaction(tx *sql.Tx, tz *entities.MstprocessEntity, processid int64) (int64, error) {
	logger.Log.Println("In side InsertMstprocessrecordmap")
	logger.Log.Println("Query -->", insertMstprocessrecordmap)
	stmt, err := tx.Prepare(insertMstprocesstemplaterecordmap)
	defer stmt.Close()
	if err != nil {
		logger.Log.Println("InsertMstprocesstemplaterecordmapwithtransaction Prepare Statement  Error", err)
		return 0, err
	}
	...
	if err != nil {
		logger.Log.Println("InsertMstprocesstemplaterecordmapwithtransaction Execute Statement  Error", err)
		return 0, err
	}
	lastInsertedId, err := res.LastInsertId()
	return lastInsertedId, nil
...
*/
func UpdateMstprocesstemplaterecordmapwithtransaction(tx *sql.Tx, tz *entities.MstprocessEntity) error {
	logger.Log.Println("In side UpdateMstprocessrecordmap")
	stmt, err := tx.Prepare(updateMstprocesstemplaterecordmap)
	defer stmt.Close()
	if err != nil {
		logger.Log.Println("UpdateMstprocesstemplaterecordmapwithtransaction Prepare Statement  Error", err)
		return err
	}
	_, err = stmt.Exec(tz.Mstorgnhirarchyid, tz.Recorddifftypeid, tz.Recorddiffid, tz.Mstprocessrecordmapid)
	if err != nil {
		logger.Log.Println("UpdateMstprocesstemplaterecordmapwithtransaction Execute Statement  Error", err)
		return err
	}
	return nil
}
...
*/
func DeleteMstprocesstemplaterecordmapwithtransaction(tx *sql.Tx, tz *entities.MstprocessEntity) error {
	logger.Log.Println("In side DeleteMstprocessrecordmap")
	stmt, err := tx.Prepare(deleteMstprocesstemplaterecordmap)
	defer stmt.Close()
	if err != nil {
		logger.Log.Println("DeleteMstprocesstemplaterecordmapwithtransaction Prepare Statement  Error", err)
		return err
	}
	_, err = stmt.Exec(tz.Mstprocessrecordmapid)
	if err != nil {
		logger.Log.Println("DeleteMstprocesstemplaterecordmapwithtransaction Execute Statement  Error", err)
		return err
	}
	return nil
}
