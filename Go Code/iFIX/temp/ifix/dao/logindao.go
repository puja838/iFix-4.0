...
var getuserdetails = "SELECT a.clientid as Clientid,a.mstorgnhirarchyid as Mstorgnhirarchyid,COALESCE(a.groupid,0) groupid,COALESCE(a.vipuser,'NA') vipuser,COALESCE(a.color,''),a.loginname as Loginname,a.name as Username,a.useremail as Email,a.usermobileno as Mobile,b.name as Mstorgnname,b.mstorgnhierarchytypeid as Orgntypeid,coalesce(b.logintypeid,0) logintypeid,c.name as Clientname,coalesce(a.branch,'NA') branch,coalesce(a.firstname,'NA') firstname,COALESCE(a.lastname,'NA') lastname from mstclientuser a,mstorgnhierarchy b,mstclient c where a.clientid=c.id and mstorgnhirarchyid=b.id  and a.id=? and a.activeflag=1 and a.deleteflag=0"
...
func (mdao DbConn) GetUserDetailsById(tz *entities.UserEntity) ([]entities.UserEntity, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&value.Clientid, &value.Mstorgnhirarchyid,&value.Deafultgroup, &value.Vipuser, &value.Color, &value.Loginname, &value.Username, &value.Email, &value.Mobile, &value.Mstorgnname, &value.Orgntypeid,&value.Logintypeid, &value.Clientname, &value.Branch, &value.Firstname, &value.Lastname)
		if err != nil {
