...
func InsertMapfunctionalitywithgroup(tz *entities.MapfunctionalitywithgroupEntity) (int64, bool, error, string) {
	...
	// 		}
	// 		if count.Total == 0 {
	// 			_, err := dataAccess.InsertMapfunctionalitywithgroup(tz, tz.Diffid[k])
	// 			if err != nil {
	// 				return 0, false, err, "Something Went Wrong"
	// 			}
...
func GetAllMapfunctionalitywithgroup(page *entities.MapfunctionalitywithgroupEntity) (entities.MapfunctionalitywithgroupEntities, bool, error, string) {
	...
	dataAccess := dao.DbConn{DB: db}
	values, err1 := dataAccess.GetAllMapfunctionalitywithgroup(page)
	if err1 != nil {
		return t, false, err1, "Something Went Wrong"
	}
	if page.Offset == 0 {
		total, err1 := dataAccess.GetMapfunctionalitywithgroupCount(page)
		if err1 != nil {
			return t, false, err1, "Something Went Wrong"
		}
		t.Total = total.Total
